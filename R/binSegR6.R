...
binSeg = R6Class(
  ...
  public = list(
    ...
    #' minimiser. Both modes return the first \eqn{k} change-points of the greedy splitting path, so the costs sum to
    #' `$getHistory()$cost[k + 1]`. This is the cost of the greedy segmentation, not necessarily the minimal cost for
    #' \eqn{k} change-points (see `Dynp`).
    #'
    ...
    segments = function(){
      ...
      mapply(function(a, b) {
        list(Start = a, End = b,
             Cost = private$.binSegModule$eval(a, b),
             Params = private$.binSegModule$get_params(a, b))
