...
std::vector<int> peltL2(const arma::mat& signal, const double penalty, const int minSize, const int jump)
{
    ...
    for (arma::uword i = 0; i < all_ends.n_elem; ++i)
    {
        ...
        for (arma::uword kLastBkp = 0; kLastBkp < nAdmissibleBkps; ++kLastBkp)
        {
            ...
            const double currentCost = cost.eval(lastBkp, end);
